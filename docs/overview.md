...
- **Client:** Next.js App Router pages with React 19 components. Global UI state lives in a persisted Zustand store. Puzzle progress is mirrored to `localStorage` on each cell change through an autosave manager to protect against refreshes/offline usage.
