...
1. **Acceptance verification** (agent-facing) — the acceptance doc for a work
   item: `docs/uat/UAT_CW-NNN.md`, template
   ...
   via `npm run test:e2e` (Playwright — pending #42; until it lands, scripted checks run via `npm test`). The rest of this standard governs this artifact.
...
1. The UAT spec for the work item: `docs/uat/` (e.g.
   `docs/uat/UAT_CW-<ID>_*.md`).
...
- For non-web targets, substitute the project's equivalent automated check
  (`npm test`) and the same "keep failure evidence" rule.
