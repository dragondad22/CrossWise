...
(managed with `Prisma + PostgreSQL`) so any schema change — new table/model, new column, new
...
1. The live schema definition (the `Prisma + PostgreSQL` schema/model files) — source of
   truth.
2. The canonical ERD / data-model diagram, kept in sync with the schema.
3. This file.
4. The project's Non-Negotiables (see `CLAUDE.md` / `- **User-data isolation**: every query is scoped to the authenticated user; a user can only read/write their own topics, lists, puzzles, and solves.
...
> The exact casing below is a common convention; if your `Prisma + PostgreSQL` or team uses
...
- Generate migrations with your migration tool (`npm run db:push`); use short
...
- The `Prisma + PostgreSQL` schema definition — source of truth.
