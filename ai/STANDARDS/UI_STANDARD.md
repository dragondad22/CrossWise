...
- **Design baseline reference:** `none (TailwindCSS; no Figma)` *(example — set your project's
...
- **Implementation token source of truth:** the project's central token/theme
  ...
  and font choice).
- Applies to all user-facing surfaces in `- `src/app` — Next.js App Router: pages + REST API routes under `src/app/api/v1`
...
- Product/brand names keep their branded styling (e.g. `CrossWise`).
...
- Layout matches the design baseline (`none (TailwindCSS; no Figma)`) where applicable.
